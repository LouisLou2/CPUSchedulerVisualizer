...
CardWidget::CardWidget(const QString&name,
           const int&priority,
                       const int&time,QWidget* parent):QWidget(parent){
    CardWidget(name,getNowTimeStr(),priority,time,time,0,parent);
}
...
CardWidget::CardWidget(const QString&name,
           ...
           const int&time,
           const int& needTime,
                       const int&status,QWidget* parent):QWidget(parent),name(name),createdTime(createdTime),time(time){
    *(this->priority)=priority;
    *(this->needTime)=needTime;
    *(this->status)=status;
    setFixedSize(270, 80);
}
CardWidget::CardWidget(const proc&p,QWidget* parent):CardWidget(p.name,p.createdTime,p.priority,p.time,p.needTime,p.status,parent){
}
