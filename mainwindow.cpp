...
void MainWindow::initProcessManager(){
  if(!backcon||!readycon||!suspendcon||!processingTable||!processTable||!detailView){
    ...
    processManager=new ProcessManager(cpupara,preparedfile,backcon,readycon,suspendcon,processingTable,processTable,detailView,this);
}

void MainWindow::initProcessFrame() {
    // 创建一个QTableWidget
    processingTable= new QTableWidget(cpupara->getCoreCount(), 4, ui->processingFrame); // 4行4列
    // 设置QTableWidget为只读
...
void MainWindow::initAddArea() {
    processTable = new QTableWidget(this);
      ...
      processTable->setColumnCount(3); // 设置表格的列数

      // 设置表头标签
      QStringList headers;
      headers << "进程名" << "占用时间" << "优先级";
      processTable->setHorizontalHeaderLabels(headers);

      addAreaTipLabel=new LabelWithTip("重复的进程名以及运行时间小于1的进程\n会被过滤而不会被创建",this);
